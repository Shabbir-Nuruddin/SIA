  ...
  return (
    <AppLayout>
      ...
      <div className="dashboard-shell p-5 md:p-8 max-w-7xl mx-auto animate-fade-in">
        {/* Compact header — greeting + chips + the two stat tiles, all in one
            slim row instead of a large boxed hero, so the session list below
            reads as the page's clear anchor rather than competing with it. */}
        <div className="mb-6 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            <div className="text-[11px] uppercase tracking-[0.28em] text-muted-foreground mb-1.5">Study workspace</div>
            <h1 className="text-2xl md:text-[28px] font-extrabold tracking-tight text-foreground leading-tight">
              {greet}, {name}
            </h1>
            <p className="text-sm text-muted-foreground mt-1.5 max-w-xl leading-relaxed">{greetTail}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 shrink-0 lg:w-[340px]">
            <div className="flex-1 rounded-2xl border border-border/70 bg-card px-4 py-3 transition-colors hover:border-border">
              <div className="text-[9px] uppercase tracking-[0.22em] text-muted-foreground">Next exam</div>
              <div className="mt-1 text-sm font-semibold text-foreground truncate">{nearestExam?.name ?? "Set your first exam"}</div>
              <div className="mt-0.5 text-xs text-muted-foreground truncate">{nearestExam ? `${days} days remaining` : "Open Exams to add dates"}</div>
            </div>
            <div className="flex-1 rounded-2xl border border-border/70 bg-card px-4 py-3 transition-colors hover:border-border">
              <div className="text-[9px] uppercase tracking-[0.22em] text-muted-foreground">Today's focus</div>
              <div className="mt-1 text-sm font-semibold text-foreground">{sessions.length === 0 ? "No sessions yet" : `${pendingCount} open tasks`}</div>
              <div className="mt-0.5 text-xs text-muted-foreground truncate">{allDone ? "All done for today" : "Tap a session to begin"}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1.5fr_0.95fr]">
          <section className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                <Link key={q.to} to={q.to} className={`quick-card ${q.color} block`}>
                  <div className="text-2xl mb-0.5">{q.emoji}</div>
                  <div className="font-display text-base leading-tight">{q.label}</div>
                </Link>
              ))}
            </div>
            ...
            {/* Today's plan — the page's main anchor: larger heading treatment,
                the session list gets full visual priority here. */}
            <div className="surface rounded-3xl p-5 md:p-6">
              <div className="flex items-center justify-between gap-3 mb-5">
                <div>
                  <h2 className="text-lg font-bold text-foreground">Today's plan</h2>
                  <div className="mt-0.5 text-sm text-muted-foreground">Focused sessions, aligned to your next exam.</div>
                </div>
                <span className="text-xs text-muted-foreground font-mono tabular shrink-0">{completedCount}/{sessions.length} complete</span>
              </div>
              ...
              {sessions.length === 0 ? (
                ...
                <div className="space-y-3">
                  {sessions.map((s, i) => {
                    ...
                    // Status drives the card's overall treatment; the subject accent
                    // (existing .subj-* left-border classes, unchanged) only applies
                    // while a session is still actionable — once it's done or skipped,
                    // the status colour is the more useful signal at a glance.
                    const cardStatusClass = isComplete
                      ? "bg-success/[0.04] border-border/40"
                      : isSkipped
                      ? "bg-card/60 opacity-60 border-border/40"
                      : isInProgress
                      ? "bg-primary/[0.03] border-border/70 shadow-sm"
                      : "bg-card border-border/70 hover:shadow-md hover:-translate-y-0.5 hover:border-primary/25";
                    const leftAccentStyle = isComplete
                      ? { borderLeftWidth: 4, borderLeftColor: "hsl(var(--success))" }
                      : isSkipped
                      ? { borderLeftWidth: 4, borderLeftColor: "hsl(var(--muted-foreground))" }
                      : undefined;

                    const statusPill = isComplete
                      ? { label: "Done", Icon: CheckCircle2, color: "hsl(var(--success))", bg: "hsl(var(--success) / 0.12)" }
                      : isInProgress
                      ? { label: "In progress", Icon: Clock, color: "hsl(var(--primary))", bg: "hsl(var(--primary) / 0.12)" }
                      : isSkipped
                      ? { label: "Skipped", Icon: SkipForward, color: "hsl(var(--muted-foreground))", bg: "hsl(var(--muted-foreground) / 0.12)" }
                      : { label: "Focus", Icon: Clock, color: "hsl(var(--accent))", bg: "hsl(var(--accent) / 0.12)" };

                    return (
                      <div
                        key={s.id}
                        {...(i === 0 ? { "data-tutorial": "first-session" } : {})}
                        className={`rounded-2xl border p-5 transition-all duration-200 ${cardStatusClass} ${!leftAccentStyle ? subjClass : ""}`}
                        style={leftAccentStyle}
                      >
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                          <span className="text-xs text-muted-foreground font-mono tabular">{formatTime(s.start_time)} – {endTime(s.start_time, s.duration_minutes)}</span>
                          <span
                            className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider shrink-0"
                            style={{ background: statusPill.bg, color: statusPill.color }}
                          >
                            <statusPill.Icon className="h-3 w-3" />{statusPill.label} · {s.duration_minutes}m
                          </span>
                        </div>

                        {meta && (
                          <div className="text-[11px] font-mono text-muted-foreground uppercase tracking-wider mb-1.5">{meta.name} · Unit {s.unit_number}</div>
                        )}
                        <div className={`text-[18px] font-semibold leading-tight text-foreground ${isComplete ? "line-through decoration-1 decoration-muted-foreground/50" : ""}`}>
                          {s.topic_name || (meta ? unitName : "Mixed practice")}
                        </div>
                        <div className="text-xs text-primary mt-1 mb-3">Method: {methodLabel[s.method] || s.method}</div>
                            ...
                            {s.subject ? (
                              <Link to={`/questions?subject=${s.subject}&unit=${s.unit_number}${s.topic_name ? `&topic=${encodeURIComponent(s.topic_name)}` : ""}`}>
                                <Button
                                  ...
                                  className="btn-primary h-9 px-4 text-sm transition-transform hover:scale-[1.02] active:scale-[0.98]"
                              ...
                              <Button onClick={() => startSession(s)} className="btn-primary h-9 px-4 text-sm transition-transform hover:scale-[1.02] active:scale-[0.98]">
                                <Play className="h-3.5 w-3.5 mr-1.5" fill="currentColor" />Start
                              </Button>
                            )}
                            {/* Complete/skip are deliberately quieter than Start — utility
                                actions pushed to the trailing edge, not competing for focus. */}
                            <div className="ml-auto flex items-center gap-1">
                              <Button
                                variant="ghost"
                                onClick={() => updateStatus(s.id, "complete")}
                                className="h-8 px-2.5 text-xs text-muted-foreground hover:text-success hover:bg-success/10"
                              >
                                <CheckCircle2 className="h-3.5 w-3.5 mr-1" />Done
                              </Button>
                              <Button
                                variant="ghost"
                                onClick={() => updateStatus(s.id, "skipped")}
                                title="Skip"
          ...
          {/* Sidebar — one unified panel with internal dividers instead of five
              separate floating cards, so it reads as a single "at a glance"
              summary rather than a stack of disconnected widgets. */}
          <aside className="lg:sticky lg:top-14 self-start">
            <div className="glass-card rounded-3xl border border-border/70 divide-y divide-border/60 overflow-hidden">
              <div className="p-5">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="space-y-3">
                        ...
                        >
                          <Button size="sm" className="btn-primary rounded-full w-full transition-transform hover:scale-[1.02] active:scale-[0.98]">
