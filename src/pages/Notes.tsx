...
/* ────────────────────────────────────────────────────────────
   ...
   { Summary, key_definitions[{term,mark_scheme,plain_english,common_mistake}],
...
interface EquationItem {
  ...
  worked_substitution: string;
}
interface VisualSummary {
  kind: "table" | "flowchart" | "diagram";
  caption: string;
  content: string;
}
interface TipItem {
...
const normaliseNotes = (raw: any): NormalisedNotes => {
  ...
  // Detect legacy shape
  const isLegacy =
    Array.isArray(raw.core_concepts) || (Array.isArray(raw.common_mistakes) && raw.common_mistakes.length);
  ...
  if (Array.isArray(raw.core_content)) {
    ...
    core_content = raw.core_concepts.flatMap((c: any) => {
      ...
      return bullets.map((b) => ({
        statement: `${c?.cluster ? c.cluster + " — " : ""}${b}`,
        worked_example: "",
        wrong_approach: "",
      }));
    });
  ...
  if (isLegacy && raw.worked_example) {
    ...
    const steps: string = Array.isArray(w?.steps)
      ? w.steps
          .map((s: any, i: number) => `Step ${i + 1}: ${s?.step ?? ""}${s?.reason ? ` — ${s.reason}` : ""}`)
          .join("\n")
      : "";
  ...
  const equations: EquationItem[] = Array.isArray(raw.equations)
    ? raw.equations.map((e: any) => ({
        ...
        variables: Array.isArray(e?.variables)
          ? e.variables.map((v: any) => ({ symbol: v?.symbol ?? "", meaning: v?.meaning ?? "", unit: v?.unit ?? "" }))
          : [],
  ...
  const visual_summary: VisualSummary | null =
    raw.visual_summary && typeof raw.visual_summary === "object"
      ? {
          kind: raw.visual_summary.kind ?? "diagram",
          caption: raw.visual_summary.caption ?? "",
          content: raw.visual_summary.content ?? "",
        }
      : null;
  ...
  if (Array.isArray(raw.examiner_tips)) {
    examiner_tips = raw.examiner_tips.map((t: any) =>
      typeof t === "string" ? { command_word: "", tip: t } : { command_word: t?.command_word ?? "", tip: t?.tip ?? "" },
    );
  }
...
const NotesPage = () => {
  ...
  useEffect(() => {
    ...
    supabase
      .from("user_subjects")
      .select("subject,unit_number,unit_name")
      .eq("user_id", user.id)
      .order("subject")
      .order("unit_number")
      .then(({ data }) => {
  ...
  const downloadPdf = async () => {
    ...
    const footer = () => {
      ...
      doc.text(
        `Make Me Revise · ${studentName} · ${sub?.name ?? ""} U${unitParam} — ${topicParam}`,
        margin,
        pageH - 24,
      );
      doc.setTextColor(0);
    ...
    if (notes.Summary) {
      writeLine("Summary", 14, true);
      writeLine(notes.Summary, 11, false, 8);
    }
    ...
    if (notes.examiner_tips.length) {
      ...
      notes.examiner_tips.forEach((t) =>
        writeLine(`• ${t.command_word ? `[${t.command_word}] ` : ""}${t.tip}`, 11, false, 2),
      );
    }
      ...
      <div className="p-6 md:p-10 max-w-7xl mx-auto animate-fade-in">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">
            Pick any topic. We'll generate {board === "cie" ? "Cambridge (CIE)" : "Edexcel"}-grade notes you can
                        ...
                        return (
                          <div key={u.unit_number}>
                            ...
                            >
                              ...
                              <span>
                                Unit {u.unit_number} · {unitMeta?.unitCode ?? ""}
                              </span>
                            </button>
                            {unitOpen && (
                              <div className="space-y-0.5 ml-3">
                                {(unitMeta?.topics ?? []).map((t) => {
                                  const active =
                                    subjectParam === code && unitParam === u.unit_number && topicParam === t;
                                  return (
                                    <button
                                      key={t}
                ...
                <div className="flex items-start justify-between gap-3 mb-6 pb-5 border-b border-border">
                  <div>
                    <div className="text-[10px] uppercase tracking-widest font-mono text-muted-foreground">
                      {SUBJECTS[subjectParam]?.name ?? ""} · Unit {unitParam}
                    </div>
                ...
                {notes.Summary && (
                  <Section title="Summary">
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: annotateHtml(formatToHtml(notes.Summary)) }}
                    />
                  </Section>
                )}
                      ...
                      {notes.key_definitions.map((d, i) => (
                        <div key={i} className="rounded-md border border-border/60 p-3 bg-secondary/20">
                          ...
                          <div
                            ...
                            dangerouslySetInnerHTML={{ __html: annotateHtml(formatToHtml(d.mark_scheme)) }}
                          />
                          {d.plain_english && (
                            <div
                              className="text-xs text-muted-foreground mt-1.5"
                              dangerouslySetInnerHTML={{ __html: formatToHtml(`In plain English: ${d.plain_english}`) }}
                            />
                          )}
                          {d.common_mistake && (
                            <div
                              className="text-xs mt-1.5 pl-2 border-l-2 border-urgent/60 text-foreground/80"
                              dangerouslySetInnerHTML={{ __html: formatToHtml(`⚠ ${d.common_mistake}`) }}
                            />
                          )}
                        </div>
                      ))}
                        ...
                        <div key={i} className="rounded-md bg-secondary/30 p-4">
                          <div className="flex items-start gap-2 mb-2">
                            ...
                            <div className="flex-1">
                              <div
                                ...
                                dangerouslySetInnerHTML={{ __html: annotateHtml(formatToHtml(c.statement)) }}
                              />
                              {typeof c.typical_marks === "number" && c.typical_marks > 0 && (
                                <span className="inline-block text-[10px] font-mono uppercase tracking-wider text-accent mt-0.5">
                          ...
                          {c.worked_example && (
                            <div className="mt-2 text-xs">
                              <div className="text-[10px] uppercase tracking-wider font-mono text-success mb-1">
                                Worked example
                              </div>
                              <div
                                className="text-foreground/90 whitespace-pre-wrap"
                                dangerouslySetInnerHTML={{ __html: annotateHtml(formatToHtml(c.worked_example)) }}
                              />
                            </div>
                          )}
                          {c.wrong_approach && (
                            <div className="mt-2 text-xs">
                              <div className="text-[10px] uppercase tracking-wider font-mono text-urgent mb-1">
                                Common wrong approach
                              </div>
                              <div
                                className="text-foreground/80"
                                dangerouslySetInnerHTML={{ __html: formatToHtml(c.wrong_approach) }}
                              />
                            </div>
                          )}
                        </div>
                        ...
                        <div key={i} className="rounded-md border border-border/60 p-3">
                          ...
                          {e.variables.length > 0 && (
                            <div className="text-xs space-y-0.5 mb-2">
                              {e.variables.map((v, j) => {
                                const cleanMeaning = String(v.meaning ?? "")
                                  .replace(/\{?\s*meaning\s*:?\s*\}?/gi, "")
                                  .trim();
                                const cleanUnit = String(v.unit ?? "")
                                  .replace(/^[\s({]+|[\s)}]+$/g, "")
                                  .trim();
                                return (
                                  <div key={j} className="flex gap-2 items-baseline flex-wrap">
                                    <span
                                      ...
                                      dangerouslySetInnerHTML={{ __html: renderMathInString(v.symbol) }}
                                    />
                                    <span className="text-muted-foreground">=</span>
                                    <span dangerouslySetInnerHTML={{ __html: renderMathInString(cleanMeaning) }} />
                                    {cleanUnit && (
                                      <span className="text-muted-foreground">
                                        (<span dangerouslySetInnerHTML={{ __html: renderMathInString(cleanUnit) }} />)
                                      </span>
                                    )}
                                  </div>
                                );
                          ...
                          {e.worked_substitution && (
                            <div className="text-xs mt-2">
                              <span className="text-[10px] uppercase tracking-wider font-mono text-success mr-2">
                                Substitution
                              </span>
                ...
                {notes.visual_summary && notes.visual_summary.content && (
                  <Section title="Visual Summary">
                    ...
                    {notes.visual_summary.content.trim().startsWith("<") ? (
                      <div
                        className="text-sm overflow-x-auto [&_td]:px-3 [&_td]:py-1.5 [&_th]:px-3 [&_th]:py-1.5 [&_th]:text-left [&_table]:w-full"
                        dangerouslySetInnerHTML={{ __html: renderMathInString(notes.visual_summary.content) }}
                      />
                    ) : (
                      <pre
                        className="text-xs font-mono bg-secondary/30 p-3 rounded-md overflow-x-auto whitespace-pre"
                        dangerouslySetInnerHTML={{ __html: renderMathInString(notes.visual_summary.content) }}
                      />
                    )}
                  </Section>
                    ...
                    <ul className="space-y-2 text-sm">
                      {notes.examiner_tips.map((t, i) => (
                        <li key={i} className="flex gap-2">
                          ...
                          <span>
                            {t.command_word && (
                              <span className="text-[10px] font-mono uppercase tracking-wider text-primary mr-2 px-1.5 py-0.5 rounded bg-primary/10">
                                {t.command_word}
                              </span>
                            )}
                    ...
                    {!showFlashcards ? (
                      ...
                      <div className="rounded-lg border border-border bg-secondary/20 p-6 text-center">
                        ...
                        <div className="text-[10px] text-muted-foreground mb-3">
                          Click card to {flashFlipped ? "see question" : "reveal answer"}
                        </div>
                        <div className="flex justify-center gap-2">
                          ...
                          <Button
                            size="sm"
                            disabled={flashIndex === notes.flashcards.length - 1}
                            onClick={() => {
                              setFlashIndex((i) => Math.min(notes.flashcards.length - 1, i + 1));
                              setFlashFlipped(false);
                            }}
                          >
                            Next →
                  ...
                  >
                    <div className="glass-card rounded-lg p-3 shadow-xl border-primary/40">
                      ...
                      <div className="flex gap-2 mt-2 justify-end">
                        ...
                        <Button size="sm" onClick={saveAnnotation} className="bg-primary hover:bg-primary/90">
                          Save
                        </Button>
                      </div>
                    </div>
    ...
    <div className="flex items-center gap-3 mb-6 pb-5 border-b border-border">
      ...
      <div>
        ...
        <div className="text-xs text-muted-foreground">
          Pulling {board === "cie" ? "Cambridge (CIE)" : "Edexcel"} mark-scheme phrasing and worked examples.
        </div>
      </div>
    </div>
